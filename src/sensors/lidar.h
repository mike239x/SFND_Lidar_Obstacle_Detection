...
struct Ray
{
    ...
    // resoultion: the magnitude of the ray's step, used for ray casting, the smaller the more accurate but the more
    // expensive
...
struct Lidar
{
    ...
    : cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0, 0, 2.6)
    {
        ...
        // DONE:: increase number of layers to 8 to get higher resoultion pcd
        int numLayers = 8;
