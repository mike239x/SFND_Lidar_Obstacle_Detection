...
Ransac(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
    ...
    for (int t = 0; t < maxIterations; ++t) // `while (maxIterations--)` from the solution is better
    {
        ...
        x /= d;
        y /= d;
        auto f = [x, y](pcl::PointXYZ p) -> float { return x * p.x + y * p.y; };
        auto c = f(p_i);
        int n  = 0;
...
RansacPlane(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
    ...
    do
    {
        ...
        if (d < 0.0000001)
            continue;
        normal /= d;
        auto f = [normal](pcl::PointXYZ p) -> float { return normal.adjoint() * p.getVector3fMap(); };
        auto c = f(p_i);
        int n  = 0;
        // Measure distance between every point and fitted line
        for (size_t k = 0; k < cloud->size(); ++k)
    ...
    auto f  = [normal, c](pcl::PointXYZ p) -> float { return std::abs(normal.adjoint() * p.getVector3fMap() - c); };
    std::unordered_set<int> inliersResult;
    for (size_t k = 0; k < cloud->size(); ++k)
    {
        auto p_k = cloud->points[k];
